...
struct Item
{
	...
	uint8_t field_3;
	int32_t modifier;
	uint8_t recipe_dummy_0;
	uint8_t recipe_dummy_1;
	uint8_t recipe_dummy_2;
	uint8_t recipe_dummy_3;
	uint8_t rarity;
	uint8_t material;
	uint8_t adapted;
	uint8_t field_F;
...
struct ReversedItemStack : Item
{
	uint32_t amount;
};

struct CraftItemStack
{
	uint32_t amount;
	Item item;
	MSVCBinCompat::vector<ItemStack> crafting_components;
};

struct CraftItem : Item
{
	uint32_t craft_type;
	MSVCBinCompat::vector<ItemStack> crafting_components;
};
...
struct zone_pickupable_object
{
	...
	zone_pickupable_object() {
		...
		*/
		...
		this->item.item_id = 0;
		this->item.recipe_dummy_0 = 0;
		this->item.recipe_dummy_1 = 0;
		this->item.recipe_dummy_2 = 0;
		this->item.recipe_dummy_3 = 0;
		this->item.rarity = 0;
